...
export default function ProjectDetailsPage() {
  ...
  const {
    ...
    loading,
    error,
  } = useAppSelector((state) => state.kitchen);
  ...
  // 2. Stabilize the Project check for the dependency array
  const hasProject = !!project;

  // 3. Reveal Animation Logic
  useEffect(() => {
    // Only run animation if we have loaded the project and haven't shown content yet
    if (!loading && hasProject && !showContent && initData) {
      const ctx = gsap.context(() => {
        ...
        // Emergency backup to prevent infinite spinning
  ...
  }, [loading, hasProject, showContent, initData]);
  ...
  return (
    ...
    >
      ...
      {project && (
        <div
          className={`flex-1 flex flex-col overflow-hidden p-4 md:p-8 lg:p-10 pt-4 transition-opacity duration-500 ${
        ...
        >
          ...
          <div className="flex-none flex items-center justify-between mb-4 animate-reveal">
            ...
            <div className="hidden xs:flex items-center gap-2">
              ...
              <span className="text-[9px] font-mono text-foreground/40 uppercase tracking-[0.3em]">
                Direct_Sync_Active
              </span>
            </div>
          </div>
          ...
          {/* Main Layout Grid */}
          <div className="flex-1 flex flex-col lg:flex-row gap-6 min-h-0 overflow-y-auto lg:overflow-hidden scrollbar-hide">
            {/* Spatial Editor Section */}
            <section className="flex-[4] flex flex-col min-h-[400px] md:min-h-0 space-y-2 animate-reveal">
              <div className="flex-none flex items-center justify-between mb-2">
                <div className="space-y-1">
                  <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground/80">
                    Spatial_Registry
                  </h2>
                  <p className="text-[9px] font-mono text-foreground/20 uppercase tracking-widest">
                    Architectural_Node_Manipulation_v4
                  </p>
                </div>
              </div>

              <div className="flex-1 glass-brilliant rounded-[1.5rem] md:rounded-[3rem] overflow-hidden border border-border bg-transparent min-h-0">
                <SpatialEditor />
              </div>
            </section>
            ...
            <div className="flex-[1] flex flex-col gap-4 lg:max-w-[320px] min-h-0">
              <div className="flex-none h-auto md:h-32 animate-reveal">
                <ProjectHero project={project} />
              </div>

              <div className="flex-1 animate-reveal min-h-0">
