        ...
        <div className="space-y-6 md:space-y-10 p-4 md:p-10 max-w-7xl mx-auto font-mono">
                ...
                <div className="space-y-2">
                    <div className="flex items-center gap-3">
                        ...
                        <h1 className="text-2xl md:text-3xl font-black uppercase tracking-tighter text-foreground italic">
                            Material_DB<span className="text-foreground/20 not-italic">.bin</span>
                        </h1>
                    </div>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4 md:gap-6">
                {[
                    ...
                    { label: "Quality_Shield", count: "100%", icon: ShieldCheck, color: "text-foreground" },
                ].map(stat => (
                    <div key={stat.label} className="glass-brilliant p-5 md:p-6 rounded-3xl border border-border space-y-4">
                        <div className={`p-2 w-fit rounded-lg bg-background/30 border border-border ${stat.color}`}>
                            <stat.icon size={16} />
                        </div>
                        <div className="space-y-1">
                            <p className="text-[8px] text-foreground/30 uppercase tracking-[0.2em]">{stat.label}</p>
                            <p className="text-xl font-black text-foreground italic">{stat.count}</p>
                        </div>
                    </div>
                ))}
            </div>
            ...
            <div className="glass-brilliant rounded-[1.5rem] md:rounded-[2.5rem] border border-border overflow-hidden">
                <div className="p-6 md:p-8 border-b border-border bg-muted/20">
                    <h2 className="text-[10px] font-black uppercase tracking-[0.4em] text-foreground/80">Stock_Registry</h2>
                </div>
                <div className="py-12 px-6 md:p-20 text-center">
                    ...
                    <button className="mt-8 w-full sm:w-auto px-8 py-3 bg-background/30 border border-border rounded-xl text-[10px] font-black uppercase tracking-widest text-foreground/40 hover:text-foreground hover:bg-background/50 transition-all">
