...
export async function createProject(prevState: FormState, formData: FormData): Promise<FormState> {
    ...
    try {
        ...
        const project = new Project({ name, client: name, status: 'Draft', progress: 0 });
        ...
        const kitchen = new Kitchen({
            ...
            walls: [{ id: `wall-${Date.now()}`, label: "Wall 1", length: 300, height: 240, thickness: 10 }],
        });
...
export async function updateKitchen(kitchenId: string, kitchenData: any): Promise<ActionState> {
    try {
        await dbConnect();
        if (!kitchenId) {
